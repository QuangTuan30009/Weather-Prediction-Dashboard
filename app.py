...
DATA_FILE = "C:\\Users\\taqua\\OneDrive\\Chuyên nghành\\Kì 4\\DAP391m\\Project\\copy9\\data\\dataset.csv"
...
DATA_FILE = "C:/Users/taqua/OneDrive/Chuyên nghành/Kì 4/DAP391m/Project/weather_prediction_web/data/dataset.csv"
...
def forecast_data():
    ...
    if date:
        ...
        for d in days:
            ...
            X = pd.DataFrame([features])
            temp = float(predictor.temp_model.predict(X)[0]) if predictor.temp_model else 0
            precipitation = float(predictor.precip_model.predict(X)[0]) if predictor.precip_model else 0
            predictions['temperature'].append(temp)
            predictions['precipitation'].append(precipitation)
            future_dates.append(d)
        forecast_data = {
...
def forecast_5days():
    ...
    df = load_data()
    data = []
    bar_colors = ["linear-gradient(90deg,#fbb034,#ffdd00)", "linear-gradient(90deg,#fbb034,#ffb347)", "linear-gradient(90deg,#fbb034,#ff7300)", "linear-gradient(90deg,#fbb034,#ff8c00)", "linear-gradient(90deg,#fbb034,#ffa500)"]
    for i, d in enumerate(days):
        ...
        if not same_days.empty:
            ...
            min_temp = temp - 2
            max_temp = temp + 2
            bar_percent = int((max_temp - 20) * 5) if max_temp else 60
            data.append({
                "label": labels[i],
                "icon": "🌧️",
                "min": int(round(min_temp)),
                "max": int(round(max_temp)),
                "barPercent": bar_percent,
                "barColor": bar_colors[i % 5]
            })
        else:
            data.append({
                "label": labels[i], "icon": "❓", "min": 25, "max": 30+i, "barPercent": 60+5*i, "barColor": "linear-gradient(90deg,#fbb034,#ffdd00)"
            })
    return jsonify({"title": "Dự báo 5 ngày", "data": data})
